#!/usr/bin/env python3
"""Q-style review board: the redesign, the splash screen, and the measurements.

    node rune-shot.mjs   /tmp/qstyle/boot2.png title     (the splash)
    node rune-shot.mjs   /tmp/qstyle/play.png  play
    node tools/render-cues.mjs > /tmp/qstyle/cues.txt
...
SRC = Path("/tmp/qstyle")
OUT = REPO / "docs" / "target" / "qstyle-board.png"
...
CAST = [
    ("wizard", 15 * 1.95 * 2.5, "player · 73px", "#7ec8f0"),
    ("brute", 27 * 1.22 * 2.5, "brute · 82px", "#ffa33d"),
    ("shade", 14 * 1.55 * 2.35, "shade · 51px", "#ff5c7a"),
    ("wisp", 12 * 1.50 * 2.5, "wisp · 45px", "#c58bff"),
    ("brazier", 104, "brazier · 82x104", "#ffd08a"),
]
...
d.text((0, 34 + sh),
       "Key art, logo, and a prompt on a panel. One press dismisses it and unlocks audio; the how-to-play screen follows.",
       font=F_S, fill=DIM)
...
d.text((0, 4), "2.  The Q-style cast at exactly the size the game draws it", font=F_H, fill=INK)
x = 4
...
# ── 3. the walk cycles ────────────────────────────────────────────────────────
try:
    pw = json.loads(Path("/tmp/art3/walk.json").read_text())
    ew = json.loads(Path("/tmp/art3/ew.json").read_text())
    rows = [
        ("player", pw),
...
gates = [
    ...
    ("hero width at 73px", "37px before -> 71px now", OK),
    ("shared canvas", "wizard 332x320, shade 274x320, brute 386x320", OK),
    ("ground line", "y=316 for every creature", OK),
    ("QA asset_report", "12/12 ok, no problems", OK),
    ("assets total", "1.62 MB (splash 209 KB, floor 174 KB)", OK),
    ("tests", "166 pass, 0 fail", OK),
